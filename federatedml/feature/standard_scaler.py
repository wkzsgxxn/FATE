...
class StandardScaler(object):
    ...
    def transform(self, data, mean, std):
        ...
        """
        if isinstance(mean, Iterable) and isinstance(std, Iterable):
            f = functools.partial(self.__scale, mean=mean, std=std)
        elif not isinstance(mean, Iterable) and not isinstance(std, Iterable):
            f = functools.partial(self.__scale_with_value, mean=mean, std=std)
        else:
